...
-- Bob doesn't have enough money, so we undo everithing
ROLLBACK;
...
BEGIN
    IF p_amount <= 0 THEN
        RAISE_APPLICATION_ERROR(-20001, 'The amount must be greater than 0');
    END IF;
...
-- Which of these should be inside the transaction? Reserve the time slot and Create the appointment record
-- Which should be outside? Send a confirmation notification Why? Because if something goes wrong, we don't want to report something that doesn't exist.
...
-- What problem does this create? It breaks the larger transaction because the procedure commits too early.
...
-- Can they? Yes 
-- Can they do the same with post_payment()? No Why or why not? because it modifies data and makes changes
